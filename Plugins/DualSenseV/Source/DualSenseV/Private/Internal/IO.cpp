...
{
	QUICK_SCOPE_CYCLE_COUNTER( DS5W_enumDevices );
...
DS5W_API void DS5W::freeDeviceContext(DS5W::DeviceContext* ptrContext) {
	...
	if (ptrContext->_internal.deviceHandle) {
		...
		ZeroMemory(os.leftTriggerEffect, sizeof(DS5W::TriggerEffect));
		os.leftTriggerEffect[0x00] = 0x05;
		ZeroMemory(os.rightTriggerEffect, sizeof(DS5W::TriggerEffect));
		os.rightTriggerEffect[0x00] = 0x05;
		os.disableLeds = true;
