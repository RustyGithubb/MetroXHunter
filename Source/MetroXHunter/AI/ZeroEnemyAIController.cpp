...
constexpr auto SPIT_TOKENS_KEYNAME = TEXT( "SpitTokens" );
constexpr auto SPIT_TOKEN_COOLDOWN_KEYNAME = TEXT( "SpitTokenCooldown" );
...
void AZeroEnemyAIController::OnPossess( APawn* InPawn )
{
	...
	Blackboard->SetValueAsInt( SPIT_TOKENS_KEYNAME, DataAsset->SpitTokens );
	Blackboard->SetValueAsFloat( SPIT_TOKEN_COOLDOWN_KEYNAME, DataAsset->SpitTokenCooldown );
...
FVector AZeroEnemyAIController::GetEQSStartLocation_Implementation() const
{
	return SpawnLocation;
}

AActor* AZeroEnemyAIController::GetEQSTargetActor_Implementation() const
{
	return GetTarget();
}
...
void AZeroEnemyAIController::TickDebugDraw()
{
	...
	Args.Add( TEXT( "bIsBulbOpen" ), CustomPawn->IsBulbOpened() );
...
{
	// arkaht: We are disconnecting this feature so it is done following our workflow with sound designers
	if ( true ) return;
...
{
	CustomPawn->Stun( CustomPawn->Data->SubstateChangeAnimationTime );
	CustomPawn->Scream();
}
