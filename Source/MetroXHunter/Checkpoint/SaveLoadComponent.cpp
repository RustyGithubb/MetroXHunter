...
void USaveLoadComponent::Save()
{
	...
	for ( auto Component : ComponentsToSave )
	{
		...
		{
			ISaveable::Execute_OnSaveData( Component, ActorGUID, GameInstance->SaveGame );
		}
	}
	...
	{
		GameInstance->SaveGame->SaveExtraProperties( ActorGUID, SavedData );
	}
...
{
	if ( !GameInstance->HasSaveGame() ) return;
