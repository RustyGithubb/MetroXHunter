...
DEFINE_LOG_CATEGORY( LogMetroLightningEmitter );
...
void ALightningEmitter::BeginPlay()
{
	...
	if ( PoolManager )
	{
		// Initialize the pool for the scorch decal with 100 instances
		if ( BP_ScorchDecals )
		{
			// Number of decals used around 90 maximum with an average of 40/30 free actors
			PoolManager->InitializePool( BP_ScorchDecals, 90 );
		}

		// Initialize the pool for the lightning projectile actor with instances (editor limit)
		if ( BP_LightningProjectile )
		{
			PoolManager->InitializePool( BP_LightningProjectile, BranchingParams.BranchesLimit );
		}
	}
...
void ALightningEmitter::DoSpawnLightning( ALightningEffect* Parent, FLightningPattern Pattern )
{
	...
	AActor* LightningEffectActor = PoolManager->GetPooledActor( BP_LightningProjectile );
	if ( !IsValid( LightningEffectActor ) ) return;

	int32 AvailableCount = PoolManager->GetAvailablePoolActor( BP_LightningProjectile );
	UE_LOG( LogTemp, Verbose, TEXT( "Available actors in pool for class '%s': %d" ), *BP_LightningProjectile->GetName(), AvailableCount );

	LightningEffectActor->SetActorTransform( Pattern.Transform );

	if ( ALightningEffect* LightningEffect = Cast<ALightningEffect>( LightningEffectActor ) )
	{
