...
class UZeroEnemySoundManagerComponent;
...
enum class EZeroEnemyState : uint8
{
	...
	UnRagdoll,
	/*
	 * Attacking a target with distance.
	 */
	SpitAttack,
	/*
...
class METROXHUNTER_API AZeroEnemy :
	public ACharacter, public IHealthHolder, public IVisualLoggerDebugSnapshotInterface, public IEQSContextProvider
{
	GENERATED_BODY()
...
public:
	...
	void KnockOut( float RagdollTime = -1.0f );
	UFUNCTION( BlueprintCallable, Category = "ZeroEnemy" )
	void UnKnockOut();
	...
	void Ragdoll();
	UFUNCTION( BlueprintCallable, Category = "ZeroEnemy" )
	void UnRagdoll( bool bBlendOverTime = true );
	...
	bool AreClawsDestroyed() const;

	// Begin IHealthHolder interface
	bool TakeDamage_Implementation( FDamageContext& DamageContext ) override;
	bool IsShownAsDamageableToPlayer_Implementation( EDamageType DamageType, const FHitResult& HitResult ) override;
...
public:
	...
	UPROPERTY( BlueprintReadWrite, Category = "ZeroEnemy" )
	bool bHasCalledParasitesOnce = false;

	UPROPERTY( BlueprintReadOnly, Category = "ZeroEnemy" )
	int32 LeftBodyPartsCount = MAX_NUM_ZERO_ENEMY_LEGS;

	UPROPERTY( EditDefaultsOnly, BlueprintReadOnly, Category = "ZeroEnemy" )
	UAnimationAsset* WakeUpAnimation = nullptr;

protected:
	UFUNCTION( BlueprintCallable, Category = "ZeroEnemy" )
	void SetBoneHidden( FName BoneName, bool bNewHidden, bool bShouldTerminate = false );
	UFUNCTION( BlueprintImplementableEvent, BlueprintCallable, Category = "ZeroEnemy" )
	void OnLimbDestroyed( FName BoneName );
...
private:
	 ...
	 */
	bool bDeadLegs[MAX_NUM_ZERO_ENEMY_LEGS] {};
	/*
	 * Specify which claw is dead or not.
	...
	UZeroEnemySoundManagerComponent* SoundManagerComponent = nullptr;
};
