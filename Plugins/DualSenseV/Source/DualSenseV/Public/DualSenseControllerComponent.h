...
class DUALSENSEV_API UDualSenseControllerComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	 ...
	 * This transpose the internal data to a useable data type.
	 * 
	 ...
	 * In contrary to GetControllerLeftStickRawValue, the length can not exceeds 1.0.
	 * 
	 ...
	 * 
	 * The length of the returned vector is in range of 0.0 : 1.0.
	 * In contrary to GetControllerRightStickRawValue, the length can not exceeds 1.0.
	 * 
	 * The amplitude that the user gives is kept.
	 */
	UFUNCTION( BlueprintPure, Category = "DualSense" )
...
public:
	...
	/*
	 * Whenever the component should automatically release the current DualSense controller
	 * during EndPlay.
	 */
	UPROPERTY( EditAnywhere, BlueprintReadOnly, Category = "DualSense" )
	bool bAutoReleaseController = true;
...
private:
	...
	uint32 LastInputUpdateFrame = INDEX_NONE;
