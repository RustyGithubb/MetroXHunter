...
void UReloadComponent::OnReloadInput()
{
	...
	if ( !UConvarLibrary::IsGunInfiniteAmmoEnabled() && CurrentAmmoAmount == 0 )
	{
		OnReloadLackAmmo.Broadcast();
		return;
	}
...
void UReloadComponent::RetrievePlayerHealth()
{
	AActor* Owner = GetOwner();
	if ( Owner )
	{
		// Get the InventoryComponent from the owner
		PlayerHealth = Owner->FindComponentByClass<UHealthComponent>();
		// Ensure the PlayerHealth is valid
		verify( IsValid( PlayerHealth ) );
	}
}
