...
void AMetroPlayerController::TickDebug_Implementation( float DeltaTime, FString& OutDebugText )
{
	...
	Args.Add( "bIsUnderAction", PlayerCharacter->bIsUnderAction );
#if UE_ENABLE_DEBUG_DRAWING
	Args.Add( "InputMode", InputModeDebugDisplayName );
#else
	Args.Add( "InputMode", TEXT( "Debug Info not available" ) );
#endif
...
void AMetroPlayerController::ToggleFreezePlayer_Implementation( bool bShouldFreeze, bool bShouldResetCameraAnimations)
{
	...
	if ( !bShouldResetCameraAnimations ) return;
	PlayerCharacter->StopPlayerCameraAnimations();
}
