...
{
	if ( PlayerCharacter->bIsUnderAction || !IsValid( CurrentInteractable ) ) return;

	if ( CurrentInteractable->bShouldPlayInteractSound )
	{
		if ( IsValid( CurrentInteractable->InteractSound ) )
		{
			UGameplayStatics::PlaySound2D( this, CurrentInteractable->InteractSound );
		}
		else
		{
